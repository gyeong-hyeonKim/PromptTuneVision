...
def create_prompt(context: dict, clip_scores) -> str:
    """
    ...
    return f"""
        ...
        CLIP Similarity Scores (frame → score):
        {clip_scores}
        ...
        2.  Then, suggest an "Improved Prompt:". This improved prompt should aim to address the issues, be clearer, more descriptive for visual elements, and increase the likelihood of all desired objects appearing in the video. And you will maximize CLIP similarity.
            {token_constraint_instruction}
        3. When you use nouns in a sentence, make sure they are only used within the nouns I told you.
...
def call_gpt(full_prompt_for_gpt: str) -> str:
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o", # 필요에 따라 "gpt-4" 등으로 변경 가능
            messages=[
